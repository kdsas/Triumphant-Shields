...
export(Resource) var item2=preload("res://Items/Flower1.tres")
...
func _on_Area2D_body_entered(body):
	if body.is_in_group('Player'):
		body.inventory.append(item2)
		if item2.name=="Cherries" and body.inventory.count(item2.name=="Cherries")<=0:
			InventoryCanvasLayer.generate_cherries_icon()
			InventoryCanvasLayer.increase_cherries()
		if item2.name=="Cherries" and body.inventory.count(item2.name=="Cherries")>0:
			InventoryCanvasLayer.increase_cherries()
		else:
			if item2.name=="Flower1" and body.inventory.count(item2.name=="Flower1")<=0:
			 InventoryCanvasLayer.generate_flowerOne_icon()
			 InventoryCanvasLayer.increase_flower1()
		if item2.name=="Flower1" and body.inventory.count(item2.name=="Flower1")>0:
			InventoryCanvasLayer.increase_flower1()
		else:
			if item2.name=="Flower2" and body.inventory.count(item2.name=="Flower2")<=0:
			 InventoryCanvasLayer.generate_flowerTwo_icon()
			 InventoryCanvasLayer.increase_flower2()
		if item2.name=="Flower2" and body.inventory.count(item2.name=="Flower2")>0:
			InventoryCanvasLayer.increase_flower2()
		else:
			if item2.name=="Loot1" and body.inventory.count(item2.name=="Loot1")<=0:
			 InventoryCanvasLayer.generate_lootOne_icon()
			 InventoryCanvasLayer.increase_loot1()
		if item2.name=="Loot1" and body.inventory.count(item2.name=="Loot1")>0:
			InventoryCanvasLayer.increase_loot1()
		else:
			if item2.name=="Loot2" and body.inventory.count(item2.name=="Loot2")<=0:
